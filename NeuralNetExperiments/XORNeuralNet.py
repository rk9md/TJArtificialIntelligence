...
def error(layer, i, nodes, weights, expected, answer): #TO MAKE FASTER STORE THE ERRORS CALCULATED
    if layer==len(weights):
        return (expected - answer[0][0])*-1####### NOT GENERALIZED
    #firstPart
    summ = 0
...
def negGradient(i,j,layer,nodes,weights,expected, answer):
    return nodes[layer][0][i]*error(layer+1,j,nodes,weights,expected, answer)
    ...
    for layer in range(len(gradients)):
        for r in range(len(gradients[layer])):
            for c in range(len(gradients[layer][r])):
                gradients[layer][r][c] = gradientScale*negGradient(r,c,layer,nodes,weights, expected, answer)
    return gradients
#################################################################################################################################
