...
def makeMatrix(row, column):
    ...
    opposite = {0:-1, 1:1}
    return [[random.random()*opposite[random.randint(0,1)] for c in range(column)] for r in range(row)]
...
#     errors = [makeMatrix(1,x) for x in actNumOfNodesInEachLayer]
#     layer = len(errors)-1
#     first = True
#     while layer!=-1:
#         if first:
#             errors[layer][0][0] = (expected - answer[0][0])
...
def error(nodes, weights, expected, answer): #  NOT GENERAL
    ...
    layer = 3
    errors[3][0][0] = (expected-answer[0][0])
    errors[2][0][0] = weights[2][0][0]* errors[3][0][0] * squashDerivative(nodes[2][0][0])
    errors[1][0][0] = weights[1][0][0]* errors[2][0][0] * squashDerivative(nodes[1][0][0])
    errors[1][0][1] = weights[1][1][0]* errors[2][0][0] * squashDerivative(nodes[1][0][1])
    errors[0][0][0] = (weights[0][0][0]* errors[1][0][0]+weights[0][0][1]* errors[1][0][1]) * squashDerivative(nodes[0][0][0])
    errors[0][0][1] = (weights[0][1][0]* errors[1][0][0]+weights[0][1][1]* errors[1][0][1]) * squashDerivative(nodes[0][0][1])
    errors[0][0][2] = (weights[0][2][0]* errors[1][0][0]+weights[0][2][1]* errors[1][0][1]) * squashDerivative(nodes[0][0][2])
    return errors
...
for x in range(iteration):
    curr = random.randint(0,len(inputs)-1)#random.randint(0,len(inputs)-1)
